...
package com.Application.robotlegs.views.packedList.listPacked
{
	 ...
	 * @see DragData
	 */
	public class DragDropListPackedManager
	{
		/**
		 * @private
		 */
		...
		private static var _limitYTopPos:Number = 0;
		private static var _limitYBottomPos:Number = 0;
		
		public static function set limitYTopPos(value:int):void{
			_limitYTopPos = value;
		}
		
		public static function set limitYBottomPos(value:int):void{
			_limitYBottomPos = value;
		}
		/**
		...
		public static function startDrag(source:IDragSource, touch:Touch, data:DragData, dragAvatar:DisplayObject = null, dragAvatarOffsetX:Number = 0, dragAvatarOffsetY:Number = 0):void
		{
			...
			_dragSource.dispatchEvent(new DragDropPackedListEvent(DragDropPackedListEvent.DRAG_START, data, false));
		...
		protected static function completeDrag(isDropped:Boolean):void
		{
			...
			if(dropTarget)
			{
				dropTarget.dispatchEvent(new DragDropPackedListEvent(DragDropPackedListEvent.DRAG_EXIT, _dragData, false, dropTargetLocalX, dropTargetLocalY));
				dropTarget = null;
			}
			var source:IDragSource = _dragSource;
			var data:DragData = _dragData;
			cleanup();
			source.dispatchEvent(new DragDropPackedListEvent(DragDropPackedListEvent.DRAG_COMPLETE, data, isDropped));
		}
		...
		protected static function updateDropTarget(location:Point):void
		{
			...
			if(target != dropTarget)
			{
				...
				if(dropTarget)
				{
					...
					dropTarget.dispatchEvent(new DragDropPackedListEvent(DragDropPackedListEvent.DRAG_ENTER, _dragData, false, dropTargetLocalX, dropTargetLocalY));
				}
			}
			else if(dropTarget)
			{
				dropTargetLocalX = location.x;
				dropTargetLocalY = location.y;
				dropTarget.dispatchEvent(new DragDropPackedListEvent(DragDropPackedListEvent.DRAG_MOVE, _dragData, false, dropTargetLocalX, dropTargetLocalY));
			}
		}
		...
		protected static function stage_touchHandler(event:TouchEvent):void
		{
			...
			if(touch.phase == TouchPhase.MOVED)
			{
				...
				//	avatar.y = HELPER_POINT.y + avatarOffsetY;
					...
					var pYPos:Number = HELPER_POINT.y + avatarOffsetY;
		...
		private static  function _droppedHelper():void{
			_touchPointID = -1;
			var isDropped:Boolean = false;
			if(dropTarget && isAccepted)
			{
				dropTarget.dispatchEvent(new DragDropPackedListEvent(DragDropPackedListEvent.DRAG_DROP, _dragData, true, dropTargetLocalX, dropTargetLocalY));
				isDropped = true;
			}
			dropTarget = null;
